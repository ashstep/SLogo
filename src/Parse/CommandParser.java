...
public class CommandParser {
    ...
    protected Node initNewNode(String commandString) {
        ...
        //uncomment this with the correct names for command info:
	...
	private Node buildTree(){
		...
		//for(int i = 0; i < commandList.size(); i++){
			...
			//TODO add in correct name for "numArgumentsmethodname" 
			for (int i = 0; i < currCommandNode.getCommandObject().numArgumentsmethodname(); i++) { 
				indexofCommand++;
	...
	private void addVariableToHashmap(String variableNameWithColon, Double valuetobeAdded){		
		if (!(variablesinCurrentCommand.containsKey(variableNameWithColon))) {
			//would we need to have a way to update the variable holding? check on site
